#!/usr/bin/env python3
"""D5: Low-DD bidirectional - Extended search with ADX threshold variants"""
...
def run_backtest(df, tp, sl, max_bars, pos, adx_thresh, rsi_long, rsi_short):
    long_cond = (
...
print("\n" + "=" * 80)
print("D5 Extended: Testing ADX 25 vs 30 with finer parameter grid")
print("=" * 80)

# Expanded grid focusing on DD < 8% with reasonable return
tp_vals = [3, 4, 5]
sl_vals = [1.5, 2, 2.5, 3]
max_bars_vals = [18, 24, 30]
pos_vals = [0.25, 0.30, 0.40]
adx_vals = [25, 30]

results = []
for tp, sl, max_bars, pos, adx_thresh in product(
    tp_vals, sl_vals, max_bars_vals, pos_vals, adx_vals
):
    # Skip high TP with very tight SL (unrealistic)
    if tp > sl * 2.5:
        continue
    result = run_backtest(df.copy(), tp, sl, max_bars, pos, adx_thresh, (30, 68), (32, 72))
    results.append(result)

# Sort by DD first, then return
results_sorted = sorted(results, key=lambda x: (x["dd"], -x["return"]))

print(f"\nTested {len(results)} valid parameter combinations")

print("\nTop 15 Low-DD configurations:")
print("-" * 110)
print(
    f"{'ADX':>4} {'TP':>3} {'SL':>4} {'MB':>4} {'POS':>5} {'Return':>8} {'DD':>6} {'WR':>6} {'R':>6} {'Exp':>8} {'Trades':>6}"
)
print("-" * 110)

for i, r in enumerate(results_sorted[:15]):
    marker = " ***" if r["return"] > 5 and r["dd"] < 8 else ""
    print(
        f"{r['adx_thresh']:>4} {r['tp']:>3} {r['sl']:>4} {r['max_bars']:>4} {r['pos']:>5.2f} {r['return']:>8.2f} {r['dd']:>6.2f} {r['win_rate']:>6.1f} {r['r_ratio']:>6.2f} {r['expectancy']:>8.4f} {r['trades']:>6}{marker}"
    )

# Find best DD < 6%
dd6_results = [r for r in results_sorted if r["dd"] <= 6.0]
dd8_results = [r for r in results_sorted if r["dd"] <= 8.0]

print(f"\nBest config with DD <= 6%:")
if dd6_results:
    best = dd6_results[0]
    print(
        f"  ADX:{best['adx_thresh']} TP:{best['tp']}% SL:{best['sl']}% MaxBars:{best['max_bars']} Pos:{best['pos'] * 100:.0f}%"
    )
    print(
        f"  Return: {best['return']:.2f}% | DD: {best['dd']:.2f}% | Win%: {best['win_rate']:.1f} | R:{best['r_ratio']}:1 | Exp:{best['expectancy']:.4f}"
    )

print(f"\nBest config with DD <= 8%:")
if dd8_results:
    best = dd8_results[0]
    print(
        f"  ADX:{best['adx_thresh']} TP:{best['tp']}% SL:{best['sl']}% MaxBars:{best['max_bars']} Pos:{best['pos'] * 100:.0f}%"
...
print(f"\n*** BEST RETURN CONFIG ***")
print(
    f"  ADX:{best_return['adx_thresh']} TP:{best_return['tp']}% SL:{best_return['sl']}% MaxBars:{best_return['max_bars']} Pos:{best_return['pos'] * 100:.0f}%"
)
print(
...
print(f"  vs V70: {best_return['vs_v70']:+.2f}%")
...
with open("user_data/reports/d5_results.json", "w") as f:
    json.dump(
        {
            "strategy": NAME,
            "best_dd_6": dd6_results[0] if dd6_results else None,
            "best_dd_8": dd8_results[0] if dd8_results else None,
