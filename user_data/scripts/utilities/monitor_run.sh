...
COMMAND="$1"
# COMMAND 包含完整 "freqtrade trade ..."，取出後面的參數部分
...
while true; do
  ...
  echo "正在啟動 freqtrade..."
  # 取得 freqtrade 路徑
  FREQTRADE_BIN="$SCRIPT_DIR/../../../.venv/bin/freqtrade"
  if command -v setsid >/dev/null 2>&1; then
    setsid zsh -c "cd $(dirname $SCRIPT_DIR)/../.. && exec $FREQTRADE_BIN $COMMAND_ARGS" &
  else
    # 回退：不使用 setsid，仍嘗試以單一 PID 追蹤
    zsh -c "cd $(dirname $SCRIPT_DIR)/../.. && exec $FREQTRADE_BIN $COMMAND_ARGS" &
  fi
  PID_TRACKER=$!
  # 取得進程組 ID（去除空白）
