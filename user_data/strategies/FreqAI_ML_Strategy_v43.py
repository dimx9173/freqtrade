# FreqAI_ML_Strategy V38 - SMMA Version
# Mission: Replace SMA with SMMA (Smoothed Moving Average) for entry conditions
#
# V38 changes from V36:
# ✅ SMMA instead of SMA for volume_ratio, atr_ratio, and volume_sma
# ✅ All other logic identical to V36
# ✅ use_exit_signal = False (kept from V36)

# V36: No regime filter - trust ML fully (2026-04-30):
# ❌ 移除 RSI>72 退出（190 trades, 25.8% win rate = 虧損根源）
# ✅ E: 幣種精簡 (5幣: BTC,ETH,SOL,XRP,LTC)
# ✅ B: ML閾值動態化 (制度適應閾值)
# ✅ A: Model替換 (LightGBM → XGBoost)
# ✅ C: Feature Engineering 簡化
# ✅ D: Training參數優化 (train_period=30天)
#
# 優化重點：
# 1. freqai_enabled = True（啟用 ML）
# 2. 放寬入場條件（volume_ratio: 1.5→1.2, pressure_ratio: 1.2→1.0）
# 3. 結合 EMA 多頭過濾（市場制度識別）
# 4. 使用 ScalpOpt_UAT 成功參數（trailing_stop, stoploss）
# 5. 15m timeframe
# 6. XGBoost + 制度適應閾值
...
class FreqAI_ML_Strategy_v43(IStrategy):
    """
    FreqAI ML Strategy V30.0 "OPTIMIZED_ML_ENSEMBLE"

    V29.0 問題修復：
    ❌ freqai_enabled = False → ✅ freqai_enabled = True
    ❌ 零交易信號（入場過嚴）→ ✅ 放寬條件
    ❌ 模擬數據 → ✅ 結合真實市場結構

    V30.0 創新點：
    ✅ 啟用 FreqAI ML 引擎
    ✅ EMA 多頭市場制度過濾（來自 ScalpOpt_UAT）
    ✅ 放寬微觀結構閾值
    ✅ ScalpOpt_UAT 成功風控參數
    ✅ 15m timeframe
    ...
    # 不使用 ROI table，完全靠 Trailing Stop
    # minimal_roi = {}
    minimal_roi = {"0": 0.02, "60": 0.015, "120": 0.01}  # Added ROI table for v30_roi test

    # 系統參數
    can_short = False  # 現貨只能做多
    ...
    def populate_exit_trend(self, DataFrame: DataFrame, metadata: dict) -> DataFrame:
        """
        V30.2 出場邏輯
        完全移除 RSI 退出！只靠 Trailing Stop + MAX_DURATION
        RSI 退出在過去回測中是主要虧損源（190 trades, 25.8% win rate）
        """

        # ❌ 移除 RSI 退出（過去勝率只有 25.8%，嚴重拖垮整體）
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        try:
            ...
            # 波動率調整
            volatility = current_candle.get("atr", 0.02) / current_rate
            volatility_multiplier = max(0.5, min(1.5, 0.02 / volatility))

            # 最終倍數
            final_multiplier = (
                base_multiplier * confidence_multiplier * regime_multiplier * volatility_multiplier
            )
            final_multiplier = max(0.3, min(2.5, final_multiplier))

            calculated_stake = proposed_stake * final_multiplier

            return max(min_stake, min(calculated_stake, max_stake))

        except Exception as e:
            logger.warning(f"Custom stake calculation failed for {pair}: {e}")
            return proposed_stake
