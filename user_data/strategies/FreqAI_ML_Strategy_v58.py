...
class FreqAI_ML_Strategy_v58(IStrategy):
    """
    FreqAI ML Strategy V58 "NO_ROI_TEST"

    V58 Mission: Test if ROI is cutting winners short
    ...
    # ===========================================
    stoploss = -0.06  # 6% 止損（V58: tighter from V53's -0.12）

    # Trailing Stop（核心！）- V58: Keep identical to V43
    trailing_stop = True
    trailing_stop_positive = 0.003  # 0.3% 啟動 trailing (V58: tighter from V53's 0.005)
    trailing_stop_positive_offset = 0.015  # 1.5% offset
    trailing_only_offset_is_reached = True

    # ===========================================
    # V58 KEY CHANGE: NO ROI - ONLY trailing stop
    # ===========================================
    minimal_roi = {}  # EMPTY - no ROI exits, only trailing stop + stoploss

    # 系統參數
    can_short = True  # V58: Enable short for futures testing
    ...
    def custom_exit(
        ...
        try:
            ...
            # V58: Keep AI exit but simplify - test if this helps or hurts
            if self.freqai_enabled:
