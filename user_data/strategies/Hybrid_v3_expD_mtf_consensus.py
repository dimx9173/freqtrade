#!/usr/bin/env python3
"""
Hybrid_v3_expD_mtf_consensus — expD: 嚴格多時框 regime 共識 on top of Hybrid_v3 base
# expD: 全時框 (15m/1h/4h) 必須 unanimously 同意 regime, 否則 transition (no-entry)
# 原始版 2/3 投票, expD 升級為 3/3 一致
...
class Hybrid_v3_expD_mtf_consensus(IStrategy):
    """
    Hybrid_v3 — Paper-Validated Dual-Mode Regime-Adaptive Architecture
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # expD: UNANIMOUS consensus — all 3 TFs must agree (sum=0 or sum=6)
        # Original: 2/3 majority (sum<=1 → ranging, sum>=4 → trending)
        # Strict: only sum==0 (all ranging) or sum==6 (all trending)
        def _consensus_regime(s: int) -> int:
            if s == 0:
                return 0  # ranging — all 3 TFs agree
            elif s == 6:
                return 2  # trending — all 3 TFs agree
            else:
                return 1  # transition (will allow weak entry or no-entry)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 保留原架構，未來可從不同方向優化
        trending_entry = (dataframe["regime"] == 2) & is_additional_check & bb_rpb_conditions
