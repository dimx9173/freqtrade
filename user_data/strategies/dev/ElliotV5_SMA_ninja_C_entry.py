...
class ElliotV5_SMA_ninja_C_entry(IStrategy):
    INTERFACE_VERSION = 2

    # C_entry: 放寬進場條件，更多交易機會
    minimal_roi = {"0": 0.12, "20": 0.06, "40": 0.03, "60": 0}
    ...
    fast_ewo = 50
    slow_ewo = 200
    # C_entry: 放寬 ewo_low（更容易進場）
    ewo_low = DecimalParameter(
        -18.0, -8.0, default=buy_params["ewo_low"], space="buy", optimize=True
    )
    ewo_high = DecimalParameter(
        2.0, 12.0, default=buy_params["ewo_high"], space="buy", optimize=True
    )
    # C_entry: 放寬 rsi_buy（更高閾值 = 更容易進場）
    rsi_buy = IntParameter(30, 55, default=buy_params["rsi_buy"], space="buy", optimize=True)
