...
class FreqAI_ML_Strategy_v81H(IStrategy):
    # HYBRID: V70 regime detection + FreqAI regression confirmation
    # freqai_enabled = True, uses XGBoostRegressor
    # Spot mode only (can_short = False)
    """
    ...
    uptrend_adx_min = DecimalParameter(20, 35, default=25, decimals=0, space="buy")
    ...
    # ===========================================
    # FREQAI TARGETS (required for regression)
    # ===========================================
    def set_freqai_targets(self, dataframe: DataFrame, metadata: dict, **kwargs) -> DataFrame:
        """
        V81H: Define FreqAI regression target.
        Target: predict 3-bar forward return (decimal form, e.g. 0.005 = 0.5%)
        This is called by FreqAI during training.
        """
        # FreqAI internally renames this to &-ml_return (not &-ml_direction)
        dataframe["&-ml_return"] = dataframe["close"].shift(-3) / dataframe["close"] - 1
        return dataframe

    # ===========================================
    # FREQAI FEATURES (required for regression)
    # ===========================================
    def feature_engineering_standard(
        self, dataframe: DataFrame, metadata: dict, **kwargs
    ) -> DataFrame:
        """
        V81H: Define FreqAI features for regression.
        Max 5 features, all must be %-prefixed.
        Features are computed WITHOUT depending on populate_indicators TA.
        """
        # RSI (computed from close)
        rsi = ta.RSI(dataframe["close"], timeperiod=14)
        dataframe["%-rsi"] = rsi / 100.0  # Normalize to 0-1

        # Bollinger Bands %B (computed from close)
        bb_upper = ta.BBANDS(dataframe["close"], timeperiod=20, nbdevup=2.0, nbdevdn=2.0)[
            "upperband"
        ]
        bb_lower = ta.BBANDS(dataframe["close"], timeperiod=20, nbdevup=2.0, nbdevdn=2.0)[
            "lowerband"
        ]
        dataframe["%-bb_pct"] = (dataframe["close"] - bb_lower) / (bb_upper - bb_lower + 1e-10)

        # EMA distance (computed from close)
        ema20 = ta.EMA(dataframe["close"], timeperiod=20)
        dataframe["%-ema_dist"] = (dataframe["close"] - ema20) / ema20

        # Volume ratio (needs volume)
        vol_sma = dataframe["volume"].rolling(20).mean()
        dataframe["%-vol_ratio"] = dataframe["volume"] / (vol_sma + 1e-10)

        # ATR volatility (needs high/low/close)
        atr = ta.ATR(dataframe, timeperiod=14)
        dataframe["%-atr_vol"] = atr / dataframe["close"]
    ...
    def populate_entry_trend(self, DataFrame: DataFrame, metadata: dict) -> DataFrame:
        """
        V81H: V70 regime-adaptive entry + FreqAI regression confirmation.
        FreqAI regression outputs raw return (e.g. -0.005 to +0.008).
        Use as regime alignment filter, NOT as probability.
        """

        # V81H: FreqAI regression raw return
        # If FreqAI didn't run, ml_direction=0 → no entries fire
        if "&-ml_return" in DataFrame.columns:
            ml_dir = DataFrame["&-ml_return"]
            if hasattr(ml_dir, "fillna"):
                ml_prediction = ml_dir.fillna(0.0)
            else:
        ...
        # ===========================================
        # UPTREND ENTRIES (Long) - V81H: regression filter
        # ===========================================
        uptrend_momentum = (
        ...
        # V81H: FreqAI regression must confirm UPTREND direction
        # ml_prediction > 0 = regression predicts price UP
        uptrend_entry = (
            (regime == "uptrend")
            & base_condition
            & price_above_vwap
            ...
            & (DataFrame["adx"] >= self.uptrend_adx_min.value)
            & uptrend_momentum
            & (ml_prediction > 0.0)  # Regression confirms UP direction
        )

        # DOWNTREND SHORT entries REMOVED (spot mode - no shorting)
        # ===========================================

        # ===========================================
        # SIDEWAYS ENTRIES (Range-bound) - spot mode LONG only
        # ===========================================
        near_lower_band = DataFrame["close"] < (DataFrame["bb_lower"] * 1.02)
        near_upper_band = DataFrame["close"] > (DataFrame["bb_upper"] * 0.98)

        # V81H: sideways ML filter - regression must show directional conviction
        ml_sideways_filter = (
            (ml_prediction.abs() > 0.001)
            if hasattr(ml_prediction, "abs")
            else (abs(ml_prediction) > 0.001)
        )
        ...
        sideways_entry_long = (
            ...
            & ml_sideways_filter
            & (ml_prediction > 0.0)  # Regression predicts bounce UP
        )

        # sideways_entry_short REMOVED (spot mode)
        # volatile_entry LONG only (spot mode - no shorting in volatile)
        volatile_entry = (
            (regime == "volatile")
            & base_condition
            & (ml_prediction > 0.003)  # Strong UP signal required
        ...
        # ===========================================
        # APPLY ENTRIES (spot mode - LONG only)
        # ===========================================
    ...
    def populate_exit_trend(self, DataFrame: DataFrame, metadata: dict) -> DataFrame:
        ...
        sideways_exit_long = (regime == "sideways") & (
            (DataFrame["bb_percent"] > 0.75) | (DataFrame["adx"] > 30)
        )

        sideways_exit_short = (regime == "sideways") & (
            (DataFrame["bb_percent"] < 0.25) | (DataFrame["adx"] > 30)
        )
