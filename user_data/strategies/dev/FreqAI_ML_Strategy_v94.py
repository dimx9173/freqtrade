...
#
# V94: Regime Detection Fixes:
# - ADX threshold 25→22 for uptrend entries
# - Sideways ADX <25→<22
# - High Vol Override only in marginal zone (ADX 15-25)
# - Uptrend prediction threshold 0.55→0.60
#
# V70 Key Features:
...
class FreqAI_ML_Strategy_v94(IStrategy):
    ...
    # ===========================================
    freqai_enabled = True
    # V94: Regime Detection fixes from Obsidian research notes
    ...
    uptrend_prediction_threshold = DecimalParameter(
        0.50, 0.75, default=0.60, decimals=2, space="buy"
    )
    uptrend_adx_min = DecimalParameter(20, 35, default=22, decimals=0, space="buy")
    ...
    def detect_market_regime(self, dataframe: DataFrame) -> DataFrame:
        ...
        # V94 FIX: High Vol Override only in marginal zone (ADX 15-25), not unconditionally
        marginal_zone = (dataframe["adx"] >= 15) & (dataframe["adx"] <= 25)
        high_vol_override = (
