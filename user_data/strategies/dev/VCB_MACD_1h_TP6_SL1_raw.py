"""
VCB_MACD_1h_TP6_SL1_raw - VCB + MACD with TP=6%, SL=1.0%, NO ROI TABLE
==========================================================================

TP=6%, SL=1.0% — 直接用 custom_exit 設定，不靠 Freqtrade ROI table
"""
...
class VCB_MACD_1h_TP6_SL1_raw(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1h"
    ...
    # 移除 ROI table，讓 Freqtrade 只靠 SL + custom_exit
    minimal_roi = {}
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (dataframe["close"] > dataframe["close"].shift(1) * 1.06)  # TP 6%
            | (dataframe["close"] < dataframe["close"].shift(1) * 0.99),  # SL 1%
            "exit_long",
        ] = 1
        return dataframe
