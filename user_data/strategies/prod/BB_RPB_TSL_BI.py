...
class BB_RPB_TSL_BI(IStrategy):
    ...
    is_optimize_r_deadfish_protection = True
    ...
    is_optimize_cofi_protection = True
    ...
    is_optimize_nfix_39_protection = True
    ...
    sell_stoploss_new = DecimalParameter(
        0.1, 1.0, default=0.5, decimals=2, optimize=is_optimize_sell_stoploss
    )
    ...
    def informative_1h_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_1h = pd.concat(
            [informative_1h, mom[["momdiv_buy", "momdiv_sell", "momdiv_coh", "momdiv_col"]]], axis=1
        )
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        sl_new = self.sell_stoploss_new.value  # 初始止損

        if current_profit > 0.2:  # 如果利潤超過20%，則設置新的止損為5%
            sl_new = 0.05
        elif current_profit > 0.1:  # 如果利潤超過10%，則設置新的止損為3%
            sl_new = 0.03
        elif current_profit > 0.06:  # 如果利潤超過6%，則設置新的止損為2%
            sl_new = 0.02
        elif current_profit > 0.03:  # 如果利潤超過3%，則設置新的止損為1.5%
            sl_new = 0.015
    ...
    def confirm_trade_entry(
        ...
        buy_max_slip = self.buy_max_slip.value
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe = pd.concat(
            [dataframe, mom[["momdiv_buy", "momdiv_sell", "momdiv_coh", "momdiv_col"]]], axis=1
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if conditions:
            dataframe.loc[is_additional_check & reduce(lambda x, y: x | y, conditions), "buy"] = 1
...
def pmax(df, period, multiplier, length, MAtype, src):
    ...
    # Use numeric values: -1 for down, 1 for up, 0 for undefined (NumPy 2.x compatibility)
    pmx = np.where((pm_arr > 0.00), np.where((mavalue < pm_arr), -1.0, 1.0), 0.0)
