...
"""
FreqAI ML Strategy V81R — Regression (raw return %)
Pairs: BTC, ETH only
Timeframe: 15m
Features: 5 max (%-prefixed)
FreqAI mode: Regression (XGBoostRegressor)
Target: raw 3-bar forward return %
Entry: prediction > 65th percentile (LONG), < 35th percentile (SHORT)
"""
...
class FreqAI_ML_Strategy_v81R(IStrategy):
    """
    FreqAI Regression Strategy.
    Predicts raw 3-bar forward return % using 5 simple features.
    """
    ...
    def set_freqai_targets(self, dataframe: pd.DataFrame, metadata: dict, **kwargs) -> pd.DataFrame:
        """
        REQUIRED by FreqAI.
        Defines the target: raw 3-bar forward return % (not classification).
        Uses `&`-prefix so FreqAI recognizes it as a target.
        """
        # 3-candle forward return (15min x 3 = 45min ahead)
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        - LONG: prediction > 65th percentile of predictions
        - SHORT: prediction < 35th percentile of predictions
        ...
        if len(pred_valid) > 0:
            p35 = pred_valid.quantile(0.35)
            p65 = pred_valid.quantile(0.65)
        else:
            p35 = -0.0001
            p65 = 0.0001

        # LONG: prediction > 65th percentile (above-average prediction)
        long_cond = valid & (prediction > p65)

        # SHORT: prediction < 35th percentile (below-average prediction)
        short_cond = valid & (prediction < p35)
