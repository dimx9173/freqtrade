...
#
# V95: FreqAI + Technical Indicator Hybrid
# - Hurst exponent for trend vs mean-reversion classification
# - Z-score entry filter (|Z| < 2)
# - RSI confirmation per regime
# - ATR-based dynamic stops (2x ATR)
#
# V70 Key Features:
...
class FreqAI_ML_Strategy_v95(IStrategy):
    ...
    # ===========================================
    stoploss = -0.10  # V95: Dynamic stops via ATR

    trailing_stop = True
    trailing_stop_positive = 0.008  # 0.8% base (V95 wider)
    trailing_stop_positive_offset = 0.020  # 2.0% offset (V95 wider)
    ...
    def calc_hurst_exponent(self, series: pd.Series, lookback: int = 50) -> float:
        """V95: Hurst exponent using variance ratio method. H > 0.5 = trending, H < 0.5 = mean-reverting"""
        try:
            prices = series.dropna().tail(lookback).values
            if len(prices) < 20:
                return 0.5
            ...
            if var_1 == 0:
                return 0.5
            returns_2 = returns[::2] if len(returns) > 1 else returns
            var_2 = np.var(returns_2) if len(returns_2) > 1 else var_1
            hurst = np.log(max(var_2, 1e-10) / max(var_1, 1e-10)) / np.log(2)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # V95 ADDITIONS: Technical Indicator Framework
        # Hurst exponent (trend vs mean-reversion)
        dataframe["hurst_exponent"] = self.calc_hurst_exponent(dataframe["close"], lookback=50)
        dataframe["is_trending"] = dataframe["hurst_exponent"] > 0.5

        # Z-score filter for entries
        z_window = 20
        dataframe["z_score"] = (
            dataframe["close"] - dataframe["close"].rolling(z_window).mean()
        ) / (dataframe["close"].rolling(z_window).std() + 1e-10)
        ...
        # V95: Dynamic ATR-based stop distance (2x ATR)
        dataframe["atr_stop_distance"] = dataframe["atr"] * 2
