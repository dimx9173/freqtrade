...
class NASOSv5_mod3(IStrategy):
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["pct_change_int"] = (dataframe["pct_change"] > 0.15).astype(int) | (
            dataframe["pct_change"] < -0.15
        ).astype(int)

        dataframe["pct_change_short"] = dataframe["close"].pct_change(periods=8)
        dataframe["pct_change_int_short"] = (dataframe["pct_change_short"] > 0.08).astype(int) | (
            dataframe["pct_change_short"] < -0.08
        ).astype(int)

        dataframe["ispumping"] = (dataframe["pct_change_int"].rolling(20).sum() >= 0.4).astype(
            "int"
        )

        dataframe["islongpumping"] = (dataframe["pct_change_int"].rolling(30).sum() >= 0.48).astype(
            "int"
        )
        ...
        dataframe["recentispumping"] = (dataframe["ispumping"].rolling(300).max() > 0) | (
            dataframe["islongpumping"].rolling(300).max() > 0
        )  # | (dataframe['isshortpumping'].rolling(300).max() > 0)
        """
        END ANTIPUMP
        """
...
class TrailingBuyStrat(NASOSv5_mod3):
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        tag = super(TrailingBuyStrat, self).custom_exit(
    ...
    def confirm_trade_exit(
        ...
        **kwargs,
        ...
        val = super(TrailingBuyStrat, self).confirm_trade_exit(
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.trailing_buy_order_enabled and self.config["runmode"].value in (
            ...
            dataframe["buy"] = 0
            if (
                not self.custom_info[metadata["pair"]]["trailing_buy"]["trailing_buy_order_started"]
                and last_candle["pre_buy"] == 1
            ):
                self.custom_info[metadata["pair"]]["trailing_buy"]["trailing_buy_order_started"] = (
                    True
                )
                self.custom_info[metadata["pair"]]["trailing_buy"]["start_trailing_price"] = (
                    last_candle["close"]
                )
                self.custom_info[metadata["pair"]]["trailing_buy"]["buy_tag"] = last_candle[
                    "buy_tag"
                ]
                ...
                if (
                    ...
                    dataframe.iloc[-1, dataframe.columns.get_loc("buy_tag")] = (
                        f"{self.custom_info[metadata['pair']]['trailing_buy']['buy_tag']} ({ratio} %)"
                    )
                    # stop trailing when buy signal ! prevent from buyin much higher price when slot is free
                    self.custom_info[metadata["pair"]]["trailing_buy"][
                        "trailing_buy_order_started"
                    ] = False
                    self.custom_info[metadata["pair"]]["trailing_buy"][
                        "trailing_buy_order_uplimit"
                    ] = 0
                    ...
                    logger.info(
                        f"price to high for {metadata['pair']} at {current_price} vs {self.custom_info[metadata['pair']]['trailing_buy']['trailing_buy_order_uplimit']}"
                    )
        elif self.trailing_buy_order_enabled:
            # FOR BACKTEST
            # PROBABLY STILL NOT WORKING
            dataframe.loc[
                (dataframe["pre_buy"] == 1) & (dataframe["pre_buy"].shift() == 0), "pre_buy_switch"
            ] = 1
            dataframe["pre_buy_switch"] = dataframe["pre_buy_switch"].fillna(0)

            dataframe["barssince_last_buy"] = (
                dataframe["pre_buy_switch"].groupby(dataframe["pre_buy_switch"].cumsum()).cumcount()
            )
            ...
            dataframe.loc[:, "close_lower"] = (
                dataframe.loc[:, "close"].expanding().apply(get_local_min)
            )
            dataframe["close_lower"] = np.where(
                dataframe["close_lower"].isna() == True,
                dataframe["close"],
                dataframe["close_lower"],
            )
            dataframe["close_lower_offset"] = dataframe["close_lower"] * (
                1 + self.trailing_buy_offset
            )
            dataframe["trailing_buy_order_uplimit"] = np.where(
                dataframe["barssince_last_buy"] < 20,
                pd.DataFrame(
                    [dataframe["close_5m_last_buy"], dataframe["close_lower_offset"]]
                ).min(),
                np.nan,
            )
            ...
            dataframe.loc[
                (
                    dataframe["barssince_last_buy"] < 20
                )  # must buy within last 20 candles after signal
            ...
            dataframe.log[
                (dataframe["trailing_buy"] == 1) & (dataframe["trailing_buy_count"] == 1), "buy"
            ] = 1
        else:  # No but trailing
            dataframe.loc[(dataframe["pre_buy"] == 1), "buy"] = 1
        return dataframe
