"""
PSV5_Hybrid_V54 - 動態市場狀態切換策略 V54
根據 ATR 波動率在 PSV5_Final_Best（突破）和 PSV1_ATR_Filter（趨勢）之間切換

V54 更新:
- ROI {0: 0.05, 30: 0.03, 60: 0.02} — 更快獲利了結
- 設計用於 3x 槓桿期貨交易
...
class PSV5_Hybrid_V54(IStrategy):
    """
    Hybrid Strategy V54: Dynamically switches between breakout (PSV5) and trend-following (PSV1)
    based on market volatility (ATR ratio).

    V54 Changes:
    - Higher ROI targets for faster profit taking: {0: 0.05, 30: 0.03, 60: 0.02}
    - Optimized for 3x leverage futures trading
    """
    ...
    # V54: Faster profit taking with higher ROI targets
    stoploss = -0.03  # Dynamic base stoploss
    minimal_roi = {
        "0": 0.05,  # 5% initial target (V54: faster profit taking)
    ...
    # ========== Stoploss Parameters (Hyperoptable) ==========
    stoploss_stoploss_base = DecimalParameter(
        -0.05, -0.01, default=-0.03, decimals=3, space="stoploss"
    )

    # ========== Trailing Stop Parameters (Hyperoptable) ==========
    trailing_trailing_positive = DecimalParameter(
        0.01, 0.04, default=0.02, decimals=2, space="trailing"
    )
    trailing_trailing_offset = DecimalParameter(
        0.02, 0.08, default=0.04, decimals=2, space="trailing"
    )

    # ========== ROI Parameters (Hyperoptable) ==========
    roi_roi_0 = DecimalParameter(0.03, 0.10, default=0.05, decimals=2, space="roi")
    roi_roi_30 = DecimalParameter(0.02, 0.06, default=0.03, decimals=2, space="roi")
    roi_roi_60 = DecimalParameter(0.01, 0.04, default=0.02, decimals=2, space="roi")
    ...
    def custom_stoploss(
        ...
        if dataframe is None or len(dataframe) < 2:
            return self.stoploss_stoploss_base.value
