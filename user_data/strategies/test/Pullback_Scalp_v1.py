# Pullback_Scalp_v1 - Pullback Entry Scalp Strategy
# ========================================================
# Task: Pullback entry (not breakout) - Trade with trend
# Logic:
#   1. Long: Price pulls back to EMA 9/21, RSI drops from overbought to 40-50
#   2. Short: Price bounces to EMA 9/21, RSI bounces from oversold to 50-60
#   3. 1h confirms trend (ADX > 25, EMA alignment intact)
#   4. 15m timeframe for execution
# ========================================================
...
class Pullback_Scalp_v1(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "15m"
    can_short = True
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[long_profit_conditions, "exit_long"] = 1

        # === Short Exit Conditions ===
        short_profit_conditions = (
            # RSI enters oversold zone
            (dataframe["rsi"] < 30)
            |
            # Price hits Bollinger lower band
            (dataframe["close"] <= dataframe["bb_lower"])
        )
        dataframe.loc[short_profit_conditions, "exit_short"] = 1
