"""
Scalp_Momentum_B_v30d - Sweet Spot
====================================
Core: Strong trend + Moderate pullback (0.2%-0.6%) + RSI + Volume + Bullish candle
Exit: Tight trailing / ATR stop
Timeframe: 5m

From v28:
- Changed pullback range from 0.15% single threshold to 0.2%-0.6% range
- Added pullback_max = 0.006 (0.6%)
- pullback_min changed from 0.0015 to 0.002 (0.2%)
"""
...
class Scalp_Momentum_B_v30d(IStrategy):
    # Fixed parameters
    stoploss = -0.02
    minimal_roi = {
        "1": 0.004,
    ...
    volume_mult = 0.75
    atr_period = 10
    pullback_min = 0.002  # 0.2% pullback (changed from 0.0015)
    pullback_max = 0.006  # 0.6% pullback (new)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Pullback between 0.2% and 0.6% (changed from single threshold)
        cond_pullback = (dataframe["pullback_pct"] >= self.pullback_min) & (
            dataframe["pullback_pct"] <= self.pullback_max
        )
