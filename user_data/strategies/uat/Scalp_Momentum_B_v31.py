"""
Scalp_Momentum_B_v31 - Bidirectional Scalping + Pin Bar Detection
=================================================================
v28 base: 5m, long-only → -3.77%, 9.1% drawdown, 87.6% win rate
v31 goal: 1m/3m bidirectional, add short entries + pin bar filter

Key changes from v28/v30e:
1. Bidirectional entries (enter_long + enter_short)
2. Short conditions mirror long with inverted logic
3. Pin bar detection for reversal signals
4. Tighter stoploss for 1m scalping
5. Adjusted RSI bounds for short side
"""
...
class Scalp_Momentum_B_v31(IStrategy):
    ...
    rsi_min_long = 35
    rsi_max_long = 72
    rsi_min_short = 30  # RSI oversold for shorts (market bottom)
    rsi_max_short = 65  # RSI lower bound for shorts
    volume_mult = 0.75
    atr_period = 10
